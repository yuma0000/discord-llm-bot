...
def format_message_info(message):
    ...
    lines.append(f"author_id: {message.author.id}")
    lines.append("")
    lines.append("===== CHANNEL =====")
    lines.append(f"channel: {message.channel}")
    lines.append(f"channel_id: {getattr(message.channel, 'id', None)}")
    lines.append("")
    lines.append("===== GUILD =====")
    lines.append(f"guild: {message.guild}")
    lines.append(f"guild_id: {getattr(message.guild, 'id', None)}")
    lines.append("")
    lines.append("===== MEMBER =====")
    lines.append(f"member: {message.member}")
    lines.append("")
    lines.append("===== MENTIONS =====")
    lines.append(f"mentions: {[str(x) for x in message.mentions]}")
    lines.append("")
    lines.append("===== ATTACHMENTS =====")
    lines.append(f"attachments: {[a.url for a in message.attachments]}")
    lines.append("")
    lines.append("===== EMBEDS =====")
    lines.append(f"embeds: {len(message.embeds)}")
    lines.append("")
    lines.append("===== REACTIONS =====")
    lines.append(f"reactions: {[str(r.emoji) for r in message.reactions]}")
    lines.append("")
    lines.append("===== COMPONENTS =====")
    lines.append(f"components: {message.components}")
    lines.append("")
    lines.append("===== FLAGS =====")
    lines.append(f"flags: {message.flags}")
    lines.append("")
    lines.append("===== REFERENCE =====")
    lines.append(f"reference: {message.reference}")
    lines.append("")
    lines.append("===== THREAD =====")
    lines.append(f"thread: {message.thread}")
    lines.append("")
    lines.append("===== INTERACTION =====")
    lines.append(f"interaction: {message.interaction}")
    lines.append("")
    lines.append("===== STICKERS =====")
    lines.append(f"stickers: {message.stickers}")
    lines.append("")
    lines.append("===== APPLICATION =====")
    lines.append(f"application_id: {message.application_id}")
    lines.append("")
    lines.append("===== WEBHOOK =====")
    lines.append(f"webhook_id: {message.webhook_id}")
    lines.append("")
    lines.append("===== SYSTEM =====")
